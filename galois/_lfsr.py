...
PolyLike = Union[int, Tuple[int], List[int], np.ndarray, FieldArray]
...
class LFSR:
    ...
    * https://www.cs.uky.edu/~klapper/pdf/galois.pdf
    ...
    def __init__(self, poly: Poly, state: PolyLike = 1, config: Literal["fibonacci", "galois"] = "fibonacci"):
        r"""
...
@overload
def berlekamp_massey(sequence: FieldArray, config: str = "fibonacci", state: Literal[False] = False) -> Poly:
    ...
@overload
def berlekamp_massey(sequence: FieldArray, config: str = "fibonacci", state: Literal[True] = True) -> Tuple[Poly, FieldArray]:
    ...
@set_module("galois")
